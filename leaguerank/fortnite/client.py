...
class FortniteClient(object):
    ...
    def __init__(self, fortnite_token, launcher_token, password, email):
        password_response = postJSON(
            settings.token,
            headers={
                'Authorization': 'basic {}'.format(launcher_token)
            },
            data={
                'grant_type': 'password',
        ...
        token_response = postJSON(
            settings.token,
            headers={
                'Authorization': 'basic {}'.format(fortnite_token),
            },
            data={
    ...
    def leaderboard(self, count=50, platform=Platform.pc, mode=GameType.solo):
        """Return list of leaderboard objects.
        Object attributes include id, name, rank, and value."""

        params = {'ownertype': '1', 'itemsPerPage': count}
        leaderboard_response = self.session.post(
            endpoint=settings.leaderboard.format(platform, mode),
            params=params)
        ...
        account_ids = '&accountId='.join(
            [entry.get('accountId') for entry in leaderboard_response.get(
                                                                    'entries')]
            )
        account_response = self.session.get(
            endpoint=settings.account.format(account_ids)
            )
        players = []
    ...
    def news():
        ...
        response = Session.get_noauth(
            endpoint=settings.news,
            headers={
                'Accept-Language': 'en',
    ...
    @staticmethod
    def server_status():
        """Check the status of the Fortnite servers.
    ...
    def patch_notes(
        ...
        """Get a list of recent patch notes for fortnite.
        Can return other blogs from epicgames.com"""
        params = {
            'category': category,
